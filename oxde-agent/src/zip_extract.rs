...
mod tests {
    ...
    fn benign_zip_extracts_successfully() {
        ...
        let dest =
            std::env::temp_dir().join(format!("oxde-agent-test-benign-{}", std::process::id()));
    ...
    fn zip_slip_entry_is_rejected() {
        ...
        let dest =
            std::env::temp_dir().join(format!("oxde-agent-test-slip-{}", std::process::id()));
        std::fs::create_dir_all(&dest).expect("create dest");
    ...
    fn oversized_archive_is_rejected() {
        ...
        let dest =
            std::env::temp_dir().join(format!("oxde-agent-test-bomb-{}", std::process::id()));
        std::fs::create_dir_all(&dest).expect("create dest");
