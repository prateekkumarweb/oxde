...
/// # Errors
///
/// Returns an error if `berth.toml` (or `$BERTH_CONFIG`) can't be read or
/// parsed.
pub fn load_berth_config() -> anyhow::Result<BerthConfig> {
    let path = std::env::var("BERTH_CONFIG").unwrap_or_else(|_| "berth.toml".to_string());
...
mod tests {
    ...
    fn tls_config_manual_mode_round_trips() {
        let tls: TlsConfig = toml::from_str(
            r#"
            mode = "manual"
            cert_path = "/etc/berth/tls/cert.pem"
            key_path = "/etc/berth/tls/key.pem"
            "#,
        )
        .unwrap();
        match tls {
            TlsConfig::Manual {
                ...
                assert_eq!(cert_path, PathBuf::from("/etc/berth/tls/cert.pem"));
                assert_eq!(key_path, PathBuf::from("/etc/berth/tls/key.pem"));
            }
