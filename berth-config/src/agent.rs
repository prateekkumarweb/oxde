...
/// # Errors
///
/// Returns an error if `berth-agent.toml` (or `$BERTH_AGENT_CONFIG`) can't be
/// read or parsed.
pub fn load_agent_config() -> anyhow::Result<AgentConfig> {
    let path =
        std::env::var("BERTH_AGENT_CONFIG").unwrap_or_else(|_| "berth-agent.toml".to_string());
    crate::load(Path::new(&path))
}

fn default_hub_addr() -> String {
    format!("127.0.0.1:{}", berth_proto::AGENT_GRPC_PORT)
}
...
mod tests {
    ...
    fn agent_token_is_required() {
        let err = toml::from_str::<AgentConfig>(r#"data_dir = "/var/lib/berth-agent""#)
    ...
    fn optional_fields_fall_back_to_defaults() {
        let config: AgentConfig = toml::from_str(
            r#"
            data_dir = "/var/lib/berth-agent"
            agent_token = "tok"
            "#,
        )
        .expect("config should parse");
        assert_eq!(config.data_dir, PathBuf::from("/var/lib/berth-agent"));
