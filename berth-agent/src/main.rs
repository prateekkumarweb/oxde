...
async fn main() -> anyhow::Result<()> {
    ...
    let config = berth_config::load_agent_config().context("failed to load configuration")?;
...
async fn route_upload_chunk(
    ...
    request_id: u64,
    req: berth_proto::hub::v1::UploadZipAndExtractRequest,
...
async fn handle_request(
    ...
    match payload {
        ...
        // Exercised only by berth-hub's own AgentLink tests, which play the
