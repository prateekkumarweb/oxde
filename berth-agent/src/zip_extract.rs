...
pub fn unpack_zip<R: Read + Seek>(
    ...
    if total_size > max_uncompressed_bytes {
        return Err("uncompressed size exceeds the configured limit".to_string());
    }
...
fn enclosed_path<R: Read>(entry: &ZipFile<'_, R>) -> Result<PathBuf, String> {
    entry
        .enclosed_name()
        .ok_or_else(|| "entry path escapes the destination directory".to_string())
}
...
mod tests {
    ...
    fn benign_zip_extracts_successfully() {
        ...
        let dest =
            std::env::temp_dir().join(format!("berth-agent-test-benign-{}", std::process::id()));
    ...
    fn zip_slip_entry_is_rejected() {
        ...
        let dest =
            std::env::temp_dir().join(format!("berth-agent-test-slip-{}", std::process::id()));
        std::fs::create_dir_all(&dest).expect("create dest");
    ...
    fn oversized_archive_is_rejected() {
        ...
        let dest =
            std::env::temp_dir().join(format!("berth-agent-test-bomb-{}", std::process::id()));
        std::fs::create_dir_all(&dest).expect("create dest");
