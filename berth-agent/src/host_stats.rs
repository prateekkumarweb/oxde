...
use berth_proto::hub::v1::HostStats;
use sysinfo::{Disks, System};

/// Runs on a blocking thread: CPU% needs two samples a fixed interval apart.
pub async fn collect(data_dir: &Path) -> anyhow::Result<HostStats> {
    let data_dir = data_dir.to_owned();
    let stats = tokio::task::spawn_blocking(move || collect_blocking(&data_dir)).await?;
    Ok(stats)
}
