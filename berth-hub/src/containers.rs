...
use berth_models::{EnvVar, RunConfig};
...
pub fn container_name(app_id: &str, deployment_id: &str) -> String {
    format!("berth-{app_id}-{deployment_id}")
}

fn agent_error(err: berth_proto::hub::v1::AgentError) -> AppError {
...
fn command_result_to_app_result(result: berth_proto::hub::v1::CommandResult) -> AppResult<()> {
    match result.result {
...
impl From<berth_proto::hub::v1::ContainerStats> for ContainerStats {
    fn from(stats: berth_proto::hub::v1::ContainerStats) -> Self {
        Self {
