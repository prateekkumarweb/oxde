...
/// The built `berth-ui` SPA. `rust-embed` serves these straight off disk at
/// runtime in debug builds instead of compiling them in, so `vp build` in
/// `berth-ui/` is enough to pick up frontend changes without a Rust rebuild;
/// release builds embed the files into the binary.
#[derive(RustEmbed)]
#[folder = "../berth-ui/dist"]
