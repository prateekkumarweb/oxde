...
impl From<berth_models::ModelError> for AppError {
    fn from(err: berth_models::ModelError) -> Self {
        match err {
            berth_models::ModelError::InvalidName(name) => Self::InvalidName(name),
            berth_models::ModelError::InvalidRepoUrl(url) => Self::InvalidRepoUrl(url),
            berth_models::ModelError::InvalidRunConfig(reason) => Self::InvalidRunConfig(reason),
            berth_models::ModelError::InvalidBuildConfig(reason) => {
                Self::InvalidBuildConfig(reason)
            }
            berth_models::ModelError::InvalidEnvVar(key) => Self::InvalidEnvVar(key),
        }
    }
}
