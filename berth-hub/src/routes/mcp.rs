...
use berth_models::{AppSource, EnvVarInput};
use rmcp::{
    ServerHandler,
...
impl BerthMcpServer {
    ...
    async fn list_apps(&self, Extension(parts): Extension<Parts>) -> Result<String, String> {
        ...
        for app in apps {
            if matches!(current_user.role, AccountRole::Admin)
                || app.has_permission(&current_user.username, berth_models::PermissionLevel::Read)
            {
    ...
    async fn get_app(
        ...
        let app = storage::get_app_by_id(&self.state, &params.app_id)
            .await
            .map_err(|e| e.to_string())?;
        authz::check_app_permission(&current_user, &app, berth_models::PermissionLevel::Read)
            .map_err(|e| e.to_string())?;
    ...
    async fn delete_app(
        ...
        let app = storage::get_app_by_id(&self.state, &params.app_id)
            .await
            .map_err(|e| e.to_string())?;
        authz::check_app_permission(&current_user, &app, berth_models::PermissionLevel::Write)
            .map_err(|e| e.to_string())?;
...
#[tool_handler(router = self.tool_router, instructions = "Manage Berth apps and deployments.")]
impl ServerHandler for BerthMcpServer {}
...
pub fn router(state: &AppState) -> Router<AppState> {
    ...
    let service = StreamableHttpService::new(
        move || Ok(BerthMcpServer::new(mcp_state.clone())),
