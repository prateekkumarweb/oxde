...
async fn authenticate(
    state: &AppState,
    request: &Request<Streaming<SessionRequest>>,
) -> Result<berth_db::models::Host, Status> {
    let header = request
        .metadata()
        .get("authorization")
...
mod tests {
    ...
    /// pinning itself is berth-agent's concern, not tested here.
