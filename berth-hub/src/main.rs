...
use anyhow::Context;
use berth_config::TlsConfig;
use berth_db::models::User;
...
async fn main() -> anyhow::Result<()> {
    ...
    let config = berth_config::load_berth_config().context("failed to load configuration")?;
    ...
    // Berth's - a relative `data_dir` (e.g. the default `./data`) would
    ...
    tracing::info!(
        ...
        "hub<->agent gRPC certificate fingerprint - set hub_tls_fingerprint in berth-agent.toml on \
         an agent connecting from a different host to pin it"
    );

    let db = berth_db::connect(&data_dir)
        .await
    ...
    match &config.tls {
        TlsConfig::Off => {
            ...
            tracing::info!("Berth server started, listening on http://{addr}");
            axum::serve(
                listener,
        ...
        TlsConfig::Manual {
            ...
            tracing::info!("Berth server started, listening on https://{addr}");
...
/// exists in `users`, one is created from `berth.toml`'s
